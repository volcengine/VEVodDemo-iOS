...
@interface VEShortDramaListViewController () <UICollectionViewDelegate, UICollectionViewDataSource, VEShortDramaDetailFeedViewControllerDataSource>
    ...
    [VEDramaDataManager requestDramaList:self.pageOffset pageSize:VEShortDramaVideoListPageCount result:^(id _Nullable responseData, NSString * _Nullable errorMsg) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (resArray && resArray.count > 0) {
                ...
                strongSelf.collectionView.mj_footer.hidden = (resArray.count < VEShortDramaVideoListPageCount);
            } else {
...
- (NSString *)nextRecommondDramaIdForDramaDetailFeedPlay:(NSString *)currentDramaId {
    for (NSInteger i = 0; i < self.dramasArray.count; i++) {
        VEDramaInfoModel *dramaInfoModel = [self.dramasArray objectAtIndex:i];
        if ([currentDramaId isEqualToString:dramaInfoModel.dramaId]) {
            if (i + 1 < self.dramasArray.count) {
                VEDramaInfoModel *retDramaInfoModel = [self.dramasArray objectAtIndex:i+1];
                return retDramaInfoModel.dramaId;
            } else {
                return nil;
            }
            break;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    dramaDetailFeedViewController.dataSource = self;
